...
void Database_load(struct Connection *conn) {
  ...
  for (i=0; i<conn->db->MAX_ROWS; i++) {
    ...
    conn->db->rows[i] = malloc(sizeof(*conn->db->rows[i]));
...
void Database_create(struct Connection *conn, int MAX_DATA, int MAX_ROWS) {
  ...
  conn->db->rows = malloc(sizeof(conn->db->rows) * MAX_ROWS);
  ...
  for(i = 0; i < MAX_ROWS; i++) {
    ...
    conn->db->rows[i]->name = memset(conn->db->rows[i]->name, ' ', conn->db->MAX_DATA);
    conn->db->rows[i]->email= malloc(conn->db->MAX_DATA);
    conn->db->rows[i]->email= memset(conn->db->rows[i]->email, ' ', conn->db->MAX_DATA);
  }
}
...
void Database_delete(struct Connection *conn, int id) {
	...
	conn->db->rows[id]->name = memset(conn->db->rows[id]->name, ' ', conn->db->MAX_DATA);
	conn->db->rows[id]->email = memset(conn->db->rows[id]->email, ' ', conn->db->MAX_DATA);
}
